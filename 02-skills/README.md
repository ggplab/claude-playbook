# 스킬: 내 반복 작업을 클로드에게 자산으로 넘기기

> **스킬(Skill)** 은 반복되는 작업 절차를 한 번 정의해 두면, 필요할 때 클로드가 알아서 꺼내 쓰는 기능입니다.

데스크톱 앱에서는 Word·PDF·PowerPoint 같은 기본 탑재 스킬을 말 한마디로 부립니다.
한 걸음 더 나아가 **나만의 스킬**을 만들면, 내가 매번 똑같이 하던 셋업·정리·변환 절차를
슬래시 명령 하나로 묶을 수 있습니다. 이 영역은 주로 **Claude Code**(터미널 속 클로드)에서 다룹니다.

## 만들어 보는 예제

| 스킬 | 한 줄 | 쓰는 사람 | 가이드 |
|------|------|------|--------|
| `/new-project` | 새 프로젝트를 일관된 네이밍·git 셋업으로 시작 | 프로젝트를 자주 새로 여는 사람 | [바로가기](./new-project/) |
| `korean-writing-style` | 보고서·메일·발표자료에서 AI 말투와 번역투를 걷어낸다 | 한국어 문서를 대외로 내보내는 모든 직장인 | [바로가기](./korean-writing-style/) |
| `meeting-agenda` | 결정 안건에 옵션을, 액션에 주체·기한을 강제하는 회의 아젠다 3종 | 회의를 주관하는 사람 | [바로가기](./meeting-agenda/) |
| `lecture-slide-make` | 강의안 JSON 한 장으로 편집 가능한 강의 덱. 도식 14종은 결정 트리로 고르고 불릿은 폴백 | 강의, 세미나, 사내 교육 자료를 만드는 사람 | [바로가기](./lecture-slide-make/) |
| `markdown-to-pdf` | 클로드가 쓴 마크다운 보고서를 표·코드 블록 그대로 PDF로 | 조사 결과를 문서로 보내는 사람 (8장 실습) | [바로가기](./markdown-to-pdf/) |
| `card-news` | 내용 JSON 한 장으로 휴대폰에서 읽히는 카드뉴스 PNG 덱. 규격 미달이면 PNG를 만들지 않는다 | 요약을 카톡·SNS로 공유하는 사람 (8장 실습) | [바로가기](./card-news/) |
| `architecture-drawio-diagram` | 로고가 큼직한 시스템 구조도를 파이썬 좌표로 그려 draw.io 파일과 PNG 동시 생성. 없으면 Mermaid 대안 | 블로그·발표에 구조도를 넣는 사람 | [바로가기](./architecture-drawio-diagram/) |

### 함께 쓰면 좋은 조합

| 흐름 | 스킬 순서 |
|------|------|
| 회의 준비 | `meeting-agenda` → `korean-writing-style` → 기본 Word 스킬로 docx |
| 조사 결과 공유 (8장) | 서브에이전트 보고서 → `markdown-to-pdf`(문서) → `card-news`(메신저용 카드) |
| 기술 블로그 글 | `architecture-drawio-diagram`(구조도) → `korean-writing-style`(본문) |
| 강의 덱, 보고 덱 | `lecture-slide-make`(구조, 도식, 생성) → `korean-writing-style`(문안) → LibreOffice PDF(배포본) |

_예제는 계속 추가됩니다._

## 스킬은 어디에 두나

Claude Code는 두 위치에서 스킬을 읽습니다.

- **개인 전역** `~/.claude/skills/<이름>/SKILL.md`: 모든 프로젝트에서 쓰임
- **프로젝트 한정** `<프로젝트>/.claude/skills/<이름>/SKILL.md`: 그 레포에서만 쓰임

`SKILL.md` 한 장이면 스킬 하나가 됩니다. 맨 위 YAML 프론트매터(`name`·`description`)와
그 아래 클로드에게 주는 지시문(절차)으로 구성됩니다. 자세한 해부는
[`new-project` 가이드](./new-project/)에서 실제 파일로 설명합니다.

---

← [플레이북 홈](../README.md)
